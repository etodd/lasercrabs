...
s32 g_fbo;
s32 g_albedo_fbo;
s32 g_albedo_buffer_highres;
s32 g_normal_buffer_highres;
s32 g_depth_buffer_highres;
s32 g_fbo_highres;
s32 g_albedo_fbo_highres;
s32 color1_buffer;
s32 color1_fbo;
s32 lighting_buffer;
s32 lighting_fbo;
s32 color2_buffer;
s32 color2_depth_buffer;
s32 color2_fbo;
...
void render_point_lights(const RenderParams& render_params, s32 type_mask, const Vec2& inv_buffer_size, u16 team_mask)
{
...
void render_spot_lights(const RenderParams& render_params, s32 fbo, RenderBlendMode blend_mode, const Vec2& inv_buffer_size, const Mat4& inverse_view_rotation_only, u16 team_mask)
{
	LoopSync* sync = render_params.sync;
...
void draw(LoopSync* sync, const Camera* camera)
{
	...
	if (Settings::supersampling)
	{
		...
		sync->write<s32>(1);
		sync->write<RenderTextureType>(RenderTextureType::Texture2D);
		sync->write<s32>(g_albedo_buffer_highres);
		...
		sync->write<s32>(1);
		sync->write<RenderTextureType>(RenderTextureType::Texture2D);
		sync->write<s32>(g_normal_buffer_highres);
		...
		sync->write<s32>(1);
		sync->write<RenderTextureType>(RenderTextureType::Texture2D);
		sync->write<s32>(g_depth_buffer_highres);
	...
	// Regular lighting
	{
...

		{
			...
			render_point_lights(render_params, (s32)PointLight::Type::Normal | (s32)PointLight::Type::Shockwave, inv_buffer_size, (u16)-1);
		}

		{
			// Spot lights
			render_spot_lights(render_params, lighting_fbo, RenderBlendMode::Additive, inv_buffer_size, inverse_view_rotation_only, (u16)-1);
		}
	...
	// SSAO
	{
		...
		// Downsample
		{
			...
			sync->write<s32>(1);
			sync->write<RenderTextureType>(RenderTextureType::Texture2D);
			sync->write<s32>(g_normal_buffer);
	...
	sync->write<RenderOp>(RenderOp::BindFramebuffer);
	sync->write<s32>(Settings::supersampling ? g_albedo_fbo_highres : color1_fbo);
