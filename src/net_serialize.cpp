...
u32 crc32(const u8* buffer, memory_index length, u32 value)
{
	value ^= 0xFFFFFFFF;
	for (memory_index i = 0; i < length; i++) 
		value = (value >> 8) ^ crc32_table[(value ^ buffer[i]) & 0xFF];
...
{
	vi_assert(bits_read == 0);

	// calculate crc32 for the whole stream
	const u32 protocol_id = NET_PROTOCOL_ID;
	u32 checksum = crc32((const u8*)&protocol_id, sizeof(u32));
