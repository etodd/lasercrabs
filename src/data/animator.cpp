...
Animator::Animator()
	: bones(), time(), animation(Asset::Nothing), armature(Asset::Nothing)
{
}
...
void Animator::update_world_transforms()
{
	...
	Animation* anim = Loader::animation(animation);
	bones.resize(anim->channels.length);
	for (int i = 0; i < bones.length; i++)
	{
		int parent = arm->bones[i].parent;
		if (parent == -1)
			bones[i] = anim->channels[i].current_transform;
		else
			bones[i] = anim->channels[i].current_transform * bones[parent];
	}
}
